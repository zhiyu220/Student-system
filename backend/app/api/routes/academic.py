...
async def list_courses(db: AsyncSession = Depends(get_db)):
    ...
    try:
        result = await db.execute(
            select(Course, User.name.label("instructor_name"))
            .outerjoin(
                CourseInstructor,
                (CourseInstructor.course_id == Course.id) & (CourseInstructor.role == "primary"),
            )
            .outerjoin(User, User.id == CourseInstructor.instructor_id)
            .order_by(Course.academic_year, Course.semester, Course.code)
        )
        rows = result.all()
